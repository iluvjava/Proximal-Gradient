...
\section{Preliminaries}\label{sec:preliminaries}
    ...
    \subsection{The Proximal Operator}
        ...
        \begin{lemma}[Proximal Operator as Resolvant of Scaled Subgradient]\label{lemma:prox_alternative_form}
            When the function $f$ is convex closed and proper, we view $\text{prox}_{\alpha, f}$ as the resolvant $(I + \alpha \partial f)^{-1}$, which is also, a single-valued operator that sometimes has a nice closed form solution to it.
        \end{lemma}
        \begin{proof}
            \begin{align*}
                \mathbf 0 &\in \partial
