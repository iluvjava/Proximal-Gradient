...
\section{Literatures Review}\label{sec:Literatures}
    ...
    \subsection*{Variants of FISTA}
        ...
        Chaning $\mathcal T_L, \theta_{k + 1}$ and $\mathcal S$, produce different variants of FISTA. 
        \begin{itemize}
            ...
            This achieves $\mathcal O(1/k^2)$ on the objective value. 
            ...
            \item [2.] A version of FISTA where the iterates have weak convergence was proved in Chambolle, Dossal \cite{chambolle_convergence_2015}. It's a variant where $\theta_{k + 1} = (n + a - 1)/a$, for $a > 2$. $\mathcal T_L$ is the same as (1.). 
        ...
        \par
        Variants (3.) is simple; unfortunately, obtaining $\sigma$ in itself could be prohibitively expensive and require knowledge about the Hessian. 
        ...
        \par
        To address the first issue, Aujol et al.\cite{aujol_fista_2022} proposed an automatic restart algorithm that achieves faster convergence without knowing the prior strong convexity (or weaker quadratic growth condition) parameter $\sigma$. 
        Later, they developed the idea into a parameter-free algorithm in their work \cite{aujol_parameter-free_2023}. 
        Earlier attempts proved a fast linear convergence rate under quadratic growth conditions by triggering the restart of FISTA based on the gradient mapping norm. 
        See \cite{alamo_gradient_2019}\cite{alamo_gradient_2019}\cite{fercoq_adaptive_2019}. 
        ...
        Finally, for a theoretical underpinning of Nesterov's generic method in his book, consult Ahn and Sra\cite{ahn_understanding_2022}. 
...
\section{Nesterov's Lower Bound Clarified}\label{sec:optimal_lower_bound}
    ...
    \subsection{First-order Method}
        ...
        \begin{definition}[First Order Method]
            ...
            We are in $\mathbb R^n$ for now. Given $x^{(0)} \in \mathbb R^n$, an iterative algorithm generates sequence of $\left(x^{(n)}\right)_{n \in \mathbb N}$ in the space. All classes $\in \GAfirst$ satisfy that 
            \begin{align*}
                x^{(k + 1)} \in \left\{x^{(0)}\right\} + 
                \text{span}\left\{\nabla f\left(x^{(i)}\right)\right\}_{i = 1}^{k - 1}.
            \end{align*}
        \end{definition}
        Let $\mathcal A^k_fx^{(0)}$ denotes the solution of the k-th iterate $x^{(k)}$ generated by an algorithm $\mathcal A \in \text{GA}^{1}$, with initial guess $x^{(0)}$.
